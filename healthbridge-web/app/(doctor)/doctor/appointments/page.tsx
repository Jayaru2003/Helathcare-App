...
import { Plus, Video, Phone, Building2, CheckCircle, XCircle } from "lucide-react";
import { formatDateTime, friendlyDate, statusVariant } from "@/lib/utils";
...
const defaultForm: Partial<CreateAppointmentDto> = {
  patientId: "", doctorId: "", scheduledAt: "",
  type: "in_person", reason: "", durationMinutes: 30, fee: 0,
};
...
export default function DoctorAppointmentsPage() {
  ...
  const [form, setForm]     = useState<Partial<CreateAppointmentDto>>(defaultForm);
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div className="flex items-center justify-between">
        ...
        <Button id="new-appointment-btn" onClick={() => setModal(true)} leftIcon={<Plus className="h-4 w-4" />}>
          New Appointment
        </Button>
      </div>
      ...
      <Card>
        {loading ? (
          ...
          <EmptyState type="calendar" title="No appointments yet" message="Create your first appointment using the button above." action={<Button size="sm" onClick={() => setModal(true)}>Book Now</Button>} />
        ) : (
          <>
            <CardSection>
              {appts.map((a) => {
      ...
      <Modal open={modal} onOpenChange={setModal} title="Book Appointment" description="Schedule a new patient appointment" size="md">
        <form onSubmit={handleCreate} className="space-y-4">
          <Input id="apt-patient-id" label="Patient ID" value={form.patientId ?? ""} onChange={e => setForm(f => ({ ...f, patientId: e.target.value }))} required placeholder="Patient UUID" />
          <Input id="apt-doctor-id"  label="Doctor ID (yours)" value={form.doctorId ?? ""} onChange={e => setForm(f => ({ ...f, doctorId: e.target.value }))} placeholder={user?.id ?? "Your doctor UUID"} />
          <Input id="apt-date"       label="Date & Time" type="datetime-local" value={form.scheduledAt ?? ""} onChange={e => setForm(f => ({ ...f, scheduledAt: e.target.value }))} required />
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input id="apt-duration" label="Duration (min)" type="number" value={String(form.durationMinutes ?? 30)} onChange={e => setForm(f => ({ ...f, durationMinutes: Number(e.target.value) }))} />
          </div>
          <Textarea id="apt-reason" label="Reason" value={form.reason ?? ""} onChange={e => setForm(f => ({ ...f, reason: e.target.value }))} required placeholder="Describe the reason for this appointment…" rows={3} />
          <Input id="apt-fee" label="Fee (USD)" type="number" value={String(form.fee ?? 0)} onChange={e => setForm(f => ({ ...f, fee: Number(e.target.value) }))} />
